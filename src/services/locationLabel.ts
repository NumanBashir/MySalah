...
const nearbyCityThresholdKm = 75;

const denmarkLocations: KnownLocation[] = [
...
export function getNearestKnownLocationLabel(
  ...
  const nearestLocation = knownLocations
    .map((location) => ({
  ...
  return nearestLocation?.distanceKm <= nearbyCityThresholdKm
